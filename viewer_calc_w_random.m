...
n_viewers=100000;               %%number of viewers
...
pundecided=5;                   %%percent of viewers who don't know who their favourite is
...
sumviewers=round((100-pundecided)/100*sumviewers);  %%subtract viewers who don't know what they want (rounded amount)
n_undecided_viewers=n_viewers-sum(sumviewers);      %%undecided viewers count

j=1;    %%reset j

while j<=n_undecided_viewers
    rand_streamer_att=ceil(100*rand(1,1));                                 %%select a random streamer
    sumviewers(1,rand_streamer_att)=sumviewers(1,rand_streamer_att)+1;      %%add this viewer to the selected streamer
    j=j+1;
end
...
%%streamer quality
% figure
% histogram(sum_attributes_viewers,100)
% 
% figure
